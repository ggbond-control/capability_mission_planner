...
namespace capability_mission_planner::offline
{
    namespace
    {
        ...
        const char *checkpoint_type(NavigationCheckpointType type)
        {
            switch (type)
            {
            case NavigationCheckpointType::Start:
                return "start";
            case NavigationCheckpointType::Task:
                return "task";
            case NavigationCheckpointType::Turn:
                return "turn";
            case NavigationCheckpointType::ResourceEntry:
                return "resource_entry";
            case NavigationCheckpointType::ResourceExit:
                return "resource_exit";
            case NavigationCheckpointType::TransitionEntry:
                return "transition_entry";
            case NavigationCheckpointType::TransitionExit:
                return "transition_exit";
            case NavigationCheckpointType::Holding:
                return "holding";
            case NavigationCheckpointType::Finish:
                return "finish";
            }
            return "unknown";
        }
                               ...
                               const OfflineMissionPlan &plan, std::size_t robot, const ExportOptions &options)
        {
            ...
            if (robot < plan.navigation_checkpoints.size())
            {
                ...
                for (const auto &point : points)
                {
                    ...
                    if (!point.task_id.empty())
                        output << ", \"task\": " << json(point.task_id);
                    if (!point.transition_id.empty())
                        output << ", \"transition_id\": " << json(point.transition_id);
                    if (!point.resource.empty())
                        output << ", \"resource\": " << json(point.resource);
                    output << "}";
                }
            }
            output << "\n      ],\n      \"traffic_events\": [";
            if (robot < plan.traffic_events.size())
            {
                ...
                for (std::size_t i = 0; i < events.size(); ++i)
                {
                    ...
                    if (!event.resource.empty())
                        output << ", \"resource\": " << json(event.resource);
        ...
        const std::vector<cv::Scalar> &colors()
        {
            static const std::vector<cv::Scalar> values{
                {35, 75, 220}, {45, 155, 35}, {210, 95, 30}, {165, 45, 170}, {30, 165, 190}, {120, 80, 35}};
            return values;
        }
            ...
            const ExportOptions &options)
        {
            ...
            for (std::size_t route_index = 0; route_index < plan.routes.size(); ++route_index)
            {
                ...
                output << "\n    }";
                if (route_index + 1U != plan.routes.size())
                    output << ',';
                output << '\n';
            }
            output << "  ]\n}\n";
        ...
        const ExportOptions &options)
    {
        ...
        for (const auto &[map_id, map] : bundle.maps)
        {
            ...
            for (const auto &transition : bundle.transitions)
            {
                ...
                const std::array<cv::Point, 4> diamond{{point + cv::Point(0, -7), point + cv::Point(7, 0),

...
namespace
{
    std::vector<GridPosition> component(const MultiMapBundle &bundle, const std::string &map_id, GridPosition anchor)
    {
    ...
    std::vector<GridPosition> map_component(const MultiMapBundle &bundle, const std::string &map_id)
    {
    ...
    std::vector<GridPosition> spread_points(const MultiMapBundle &bundle, const std::string &map_id, std::size_t count)
    {
        ...
        while (selected.size() < count)
        {
            ...
            for (const auto &candidate : candidates)
            {
                ...
                for (const auto &existing : selected)
                    nearest = std::min(nearest, std::abs(candidate.x - existing.x) + std::abs(candidate.y - existing.y));
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        const auto first_points = spread_points(*bundle, first, bundle->maps.size() == 1U ? 7U : 5U);
        const std::vector<GridPosition> starts{first_points[0], first_points[1], first_points[2]};
        ...
        std::vector<MappedTask> tasks{
            ...
            make_mapped_task("T6-fire-photo", last_points[1], {"fire", "camera"}, "fire_documentation", 5)};
